...
async function createRoom() {
  ...
  try { await multiGameStore.createRoom(config.quizType, config.bestOf, config.difficulty); }
...
async function randomMatch() {
  if (multiGameStore.roomState) { promptStillInRoom(); return; }
  try { await multiGameStore.joinQueue(config.quizType, config.difficulty, config.bestOf); }
